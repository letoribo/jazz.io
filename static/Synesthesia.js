...
(function(root) {
	...
	root.map = function(type) { 
		...
		var opacity=$('debug1').innerHTML;//console.log(opacity);
		type=selSyn[selSyn.selectedIndex].innerHTML;//console.log(type);
		var syn = root.data; //console.log(root.data[type]);
		var colors = syn[type];
		for (var note = 0; note <= 127; note ++) { // creates mapping for 127 notes
			if (colors.data) { 
				data[note] = {
				...
				if (H == S && S == L) clr = blend(parray, colors[note % 12]);
				var amount = L / 10;
